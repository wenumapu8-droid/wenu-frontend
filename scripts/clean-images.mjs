...
function targetWidth(file) {
  ...
  if (file.includes('/categories/')) return TILE_MAX_W;
  if (file.includes('/truckee/')) return TILE_MAX_W;
  if (file.includes('/brand/')) return TILE_MAX_W;
  if (file.includes('/products/')) return TILE_MAX_W;
  return TILE_MAX_W;
}
