...
try {
  for (const profile of profiles) {
    const context = await browser.newContext({ viewport: { width: profile.width, height: profile.height }, reducedMotion: profile.reducedMotion, isMobile: profile.isMobile || false, hasTouch: profile.hasTouch || false, colorScheme: 'dark' });
    ...
    try {
      ...
      report.profiles.push({ profile: profile.key, status, screenshot, pass: failures.length === 0, failures, pageErrors, sharedProgressReadoutRequired: !profile.isMobile, sharedProgressReadout: facts.sharedReadoutText.trim(), sharedSignal: facts.sharedSignalText.trim(), sharedSignalVisible: facts.sharedSignalVisible, sharedSignalPressed: facts.sharedSignalPressed, signalCycle, observationSourcePresent: facts.observationSourcePresent, metrics: { width: facts.width, scrollWidth: facts.scrollWidth, bodyHeight: facts.bodyHeight, viewportHeight: facts.viewportHeight } });
