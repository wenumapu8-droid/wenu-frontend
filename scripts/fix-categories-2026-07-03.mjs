#!/usr/bin/env node
// Fix postbuild audit-catalog FAILs: 3 WM-HAN-* published with only legacy [Hanger 74],
// missing canonical [Hangers / Weights 430]. Convention (rest of HAN catalog): keep both.
// Verified vs NocoDB snapshot 2026-07-03: the 3 SKUs exist with matching titles.
...
const logf = path.join(ROOT, 'scripts', '.fix-categories-2026-07-03.log');
const log = m => { const l = `[${ts()}] ${m}`; console.log(l); fs.appendFileSync(logf, l + '\n'); };
const sleep = ms => new Promise(r => setTimeout(r, ms));

const FIXES = [
  { id: 2796, sku: 'WM-HAN-034', categories: [{ id: 74 }, { id: 430 }] },
  { id: 2782, sku: 'WM-HAN-016', categories: [{ id: 74 }, { id: 430 }] },
  { id: 2780, sku: 'WM-HAN-015', categories: [{ id: 74 }, { id: 430 }] },
];
...
async function main() {
  ...
  if (APPLY) {
    const bf = path.join(ROOT, 'data', 'backups', `fix-categories-2026-07-03-${Date.now()}.json`);
