...
async function navigate(page, suffix = '') {
  const response = await page.goto(new URL(`/kodex/lab/holocore-webgl-toroidal/${suffix}`, baseURL).toString(), { waitUntil:'domcontentloaded', timeout:30_000 });
...
async function living(name, viewport, mobile=false){
  ...
  try{await navigate(page);const before=await fingerprint(page);const beforeM=await metrics(page);await page.waitForTimeout(760);const after=await fingerprint(page);const v=await metrics(page);assertCommon(v,name);assert(v.rendererMode==='webgl-source',`${name}: WebGL source mode unavailable`);assert(before!==after,`${name}: field did not advance`);assert(v.frameCount>beforeM.frameCount,`${name}: frame count did not advance`);const screenshot=`${name}.png`;await page.screenshot({path:path.join(outputDir,screenshot),fullPage:false});report.acceptance.push({name,pass:true,before,after,metrics:v,screenshot});}catch(e){fail(name,e);}finally{await context.close();}
}
...
async function reduced(){
  ...
  try{await navigate(page);const before=await fingerprint(page);const m1=await metrics(page);await page.waitForTimeout(760);const after=await fingerprint(page);const m2=await metrics(page);assertCommon(m2,name);assert(m2.rendererMode==='webgl-source',`${name}: WebGL source mode unavailable`);assert(before===after,`${name}: reduced-motion frame moved`);assert(m1.frameCount===m2.frameCount,`${name}: frame count advanced`);assert(m1.shaderTime===m2.shaderTime,`${name}: shader time advanced`);const screenshot=`${name}.png`;await page.screenshot({path:path.join(outputDir,screenshot),fullPage:false});report.acceptance.push({name,pass:true,fingerprint:before,metrics:m2,screenshot});}catch(e){fail(name,e);}finally{await context.close();}
}

async function fallback(){
  const name='toroidal-canvas-fallback'; const context=await browser.newContext({viewport:{width:1000,height:720},colorScheme:'dark'}); const page=await context.newPage();
  try{await navigate(page,'?fallback=canvas');const v=await metrics(page);assertCommon(v,name);assert(v.rendererMode==='canvas-fallback',`${name}: fallback mode unavailable`);assert(v.fallbackFrame==='static-torus',`${name}: wrong fallback frame ${v.fallbackFrame}`);assert(v.forcedFallback==='true',`${name}: fallback was not explicit`);const screenshot=`${name}.png`;await page.screenshot({path:path.join(outputDir,screenshot),fullPage:false});report.acceptance.push({name,pass:true,metrics:v,screenshot});}catch(e){fail(name,e);}finally{await context.close();}
}
