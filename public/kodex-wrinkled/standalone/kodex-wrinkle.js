const root = document.querySelector("[data-kdx-wrinkle]");
...
const creaseLabel = root?.querySelector("[data-kdx-crease]");
const fpsLabel = root?.querySelector("[data-kdx-fps]");

if (!(canvas instanceof HTMLCanvasElement)) {
  throw new Error("KDX_WRINKLED_REALITY: canvas not found.");
}
...
const [vertexSource, fragmentSource] = await Promise.all([
  ...
  fetch("../shaders/wrinkled-reality.frag.glsl").then((response) => response.text()),
]);
...
function triggerCrease(event) {
  runtime.targetPointer =
    pointerToNormalized(event);

  runtime.creaseOrigin =
    [...runtime.targetPointer];

  runtime.creaseStartedAt =
    performance.now();

  if (creaseLabel) {
    creaseLabel.textContent = "ACTIVE";
  }
...
function activate() {
  ...
  if (creaseLabel) {
    creaseLabel.textContent = "COLLAPSE";
  }
...
function render(now) {
  ...
  gl.uniform2f(
    uniforms.u_creaseOrigin,
    runtime.creaseOrigin[0],
    runtime.creaseOrigin[1]
  );

  gl.uniform1f(
    uniforms.u_creaseAge,
    creaseAge
  );
