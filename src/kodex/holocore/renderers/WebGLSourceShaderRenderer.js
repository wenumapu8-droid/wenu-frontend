...
export const WEBGL_SOURCE_SHADER_SPEC = Object.freeze({
  id: 'observe-source-field', rendererKind: 'webgl-shader', shaderRole: 'SOURCE_GENERATOR',
  ...
  temporalContract: 'AMBIENT_UNCLOSED', seamlessLoopClaim: false, targetFps: 30, dprCap: 1.5,
  reducedMotion: 'STATIC_TIME_FRAME', fallback: 'STATIC_CANVAS_RETICLE', provenance: 'KODEX_INTERNAL_SHADER_REUSE',
});

export const WEBGL_TOROIDAL_SHADER_SPEC = Object.freeze({
  id: 'toroidal-field-webgl-v0', rendererKind: 'webgl-shader', shaderRole: 'SOURCE_GENERATOR',
  sourceVertex: 'src/kodex/observe-v2/shaders/fullscreen.vert.glsl',
  sourceFragment: 'src/kodex/holocore/shaders/toroidal-field.frag.glsl',
  temporalContract: 'AMBIENT_UNCLOSED', seamlessLoopClaim: false, targetFps: 30, dprCap: 1.5,
  reducedMotion: 'STATIC_TIME_FRAME', fallback: 'STATIC_CANVAS_TORUS',
  provenance: 'KODEX_SYNTHETIC_PERCEPTUAL_BENCHMARK', epistemic: 'ART / COMP / SPEC',
});
...
function compileShader(gl, type, source, label) {
  const shader = gl.createShader(type); if (!shader) throw new Error(`${label}: unable to create shader`);
  gl.shaderSource(shader, source); gl.compileShader(shader);
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) { const log = gl.getShaderInfoLog(shader) || 'unknown compile error'; gl.deleteShader(shader); throw new Error(`${label}: ${log}`); }
  return shader;
}
function createProgram(gl, fragmentSource) {
  ...
  const program = gl.createProgram(); if (!program) throw new Error('HoloCore WebGL: unable to create program');
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) { const log = gl.getProgramInfoLog(program) || 'unknown link error'; gl.deleteProgram(program); throw new Error(`HoloCore WebGL program: ${log}`); }
  return program;
}
...
function drawFallback(canvas, variant) {
  ...
  const width = Math.max(1, canvas.width), height = Math.max(1, canvas.height), cx = width * .5, cy = height * .5, scale = Math.min(width, height);
  ctx.fillStyle = '#05030b'; ctx.fillRect(0, 0, width, height);
  if (variant === 'toroidal') {
    ctx.lineWidth = Math.max(1, scale * .003);
    for (let i = 0; i < 18; i += 1) { const p = i / 17; ctx.strokeStyle = p > .55 ? '#b891ff' : '#553577'; ctx.globalAlpha = .16 + p * .32; ctx.beginPath(); ctx.ellipse(cx, cy + (p - .5) * scale * .05, scale * (.31 + p * .12), scale * (.15 + p * .055), 0, 0, Math.PI * 2); ctx.stroke(); }
    ctx.strokeStyle = '#d9c9ff'; ctx.globalAlpha = .78; ctx.beginPath(); ctx.ellipse(cx, cy, scale * .13, scale * .075, 0, 0, Math.PI * 2); ctx.stroke(); canvas.dataset.fallbackFrame = 'static-torus';
  } else {
    ctx.strokeStyle = '#714db8'; ctx.globalAlpha = .45; ctx.lineWidth = Math.max(1, scale * .0018);
    for (const radius of [.16,.27,.39]) { ctx.beginPath(); ctx.ellipse(cx, cy, scale * radius, scale * radius * .55, 0, 0, Math.PI * 2); ctx.stroke(); }
    ctx.strokeStyle = '#71d9e9'; ctx.globalAlpha = .7; ctx.beginPath(); ctx.moveTo(cx - scale * .36, cy); ctx.quadraticCurveTo(cx, cy - scale * .2, cx + scale * .36, cy); ctx.quadraticCurveTo(cx, cy + scale * .2, cx - scale * .36, cy); ctx.stroke();
    ctx.fillStyle = '#d9c9ff'; ctx.globalAlpha = .86; ctx.beginPath(); ctx.arc(cx, cy, scale * .042, 0, Math.PI * 2); ctx.fill(); canvas.dataset.fallbackFrame = 'static-reticle';
  }
...
export class WebGLSourceShaderRenderer {
  constructor(canvas, options = {}) {
    ...
    this.canvas = canvas; this.variant = options.variant && SHADER_VARIANTS[options.variant] ? options.variant : 'observe'; this.variantConfig = SHADER_VARIANTS[this.variant]; this.spec = this.variantConfig.spec;
    this.reducedMotion = options.reducedMotion ?? matchMedia('(prefers-reduced-motion: reduce)').matches; this.forceFallback = Boolean(options.forceFallback);
    this.targetFps = options.targetFps ?? this.spec.targetFps; this.frameInterval = 1000 / this.targetFps; this.dprCap = options.dprCap ?? this.spec.dprCap; this.staticTime = options.staticTime ?? 6.75;
    this.seed = options.seed ?? 11.73; this.state = options.state ?? .78; this.transition = options.transition ?? .86; this.intensity = options.intensity ?? .72; this.particleScale = options.particleScale ?? .68;
    ...
    this.gl = this.forceFallback ? null : canvas.getContext('webgl2', { alpha:false, antialias:false, depth:false, stencil:false, preserveDrawingBuffer:true, powerPreference:'high-performance' });
    if (!this.gl) {
      this.mode = 'canvas-fallback'; this.resizeCanvasFallback(); drawFallback(this.canvas, this.variant);
      this.canvas.dataset.rendererMode = this.mode; this.canvas.dataset.temporalContract = this.spec.temporalContract; this.canvas.dataset.seamlessLoopClaim = String(this.spec.seamlessLoopClaim); this.canvas.dataset.shaderVariant = this.variant; this.canvas.dataset.forcedFallback = String(this.forceFallback); return;
    }
    const gl = this.gl; this.program = createProgram(gl, this.variantConfig.fragment); this.uniforms = getUniformLocations(gl, this.program); this.vao = gl.createVertexArray(); this.buffer = gl.createBuffer();
    ...
    this.mode = 'webgl-source'; this.canvas.dataset.rendererMode = this.mode; this.canvas.dataset.temporalContract = this.spec.temporalContract; this.canvas.dataset.seamlessLoopClaim = String(this.spec.seamlessLoopClaim); this.canvas.dataset.targetFps = String(this.targetFps); this.canvas.dataset.shaderVariant = this.variant; this.canvas.dataset.forcedFallback = 'false';
