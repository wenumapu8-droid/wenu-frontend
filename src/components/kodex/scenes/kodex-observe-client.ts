...
import fragmentSource from '../../../kodex/shaders/observe.frag.glsl?raw';
...
class KodexObserveController {
  ...
  constructor(root: HTMLElement) {
    ...
    this.canvas = canvas;
    this.debugNode = root.querySelector<HTMLOutputElement>('[data-kdx-debug]');
    ...
    this.bindEvents();
    this.resize();
    if (this.debugEnabled) this.debugNode?.classList.add('is-visible');
  ...
  private bindEvents(): void {
    ...
    const protocol = this.root.querySelector<HTMLButtonElement>('[data-kdx-protocol]');
    ...
    protocol?.addEventListener('click', () => {
      this.setState('aware');
      this.root.querySelector<HTMLElement>('[data-kdx-message]')?.scrollIntoView({ block: 'nearest' });
    });
  ...
  private updateDebug(elapsed: number): void {
    if (!this.debugEnabled || !this.debugNode) return;
    this.debugNode.textContent = [
      `state=${this.root.dataset.state ?? 'dormant'}`,
      `time=${elapsed.toFixed(2)}`,
      `pointer=${this.pointer.x.toFixed(2)},${this.pointer.y.toFixed(2)}`,
      `audio=${this.audio.low.toFixed(2)}/${this.audio.mid.toFixed(2)}/${this.audio.high.toFixed(2)}`,
...
export function mountKodexObserveScenes(): void {
  document.querySelectorAll<HTMLElement>('[data-kdx-observe]').forEach((root) => {
    ...
    try {
      const controller = new KodexObserveController(root);
      controllers.set(root, controller);
      window.kodexObserve = {
        setAudioBands: (low: number, mid: number, high: number) => controller.setAudioBands({ low, mid, high }),
        setState: (state: SceneState) => controller.setState(state),
      };
    } catch (error) {
      console.error(error);
