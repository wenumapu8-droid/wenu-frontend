...
You are the single write owner for files explicitly allowed by the current WorkOrder. Do not edit outside that scope.

Before editing:
1. read the WorkOrder;
2. inspect current code for an existing mechanism;
3. inspect relevant tests and governed reference constraints;
4. state the exact files you intend to modify;
5. stop if write boundaries are ambiguous.
...
- deterministic behavior where an existing contract requires it;
- no new architecture merely because it is interesting to build.

Never commit, push, merge, deploy, change DNS, touch secrets, modify WooCommerce data, rewrite canon or delete unrelated work.

Run only validation commands needed for the bounded change and report exact results. A build PASS does not prove visual fidelity.
...
If the task requires a creator/canon decision, stop implementation and return NEEDS_OCIN rather than improvising.