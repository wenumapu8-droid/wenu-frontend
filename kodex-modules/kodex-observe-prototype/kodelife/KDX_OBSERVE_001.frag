...
vec3 palette(float signal) {
  vec3 deep = vec3(0.018, 0.010, 0.030);
  vec3 violet = vec3(0.35, 0.08, 0.78);
  vec3 electric = vec3(0.72, 0.46, 1.0);
  vec3 white = vec3(0.92, 0.90, 0.97);
  return mix(deep, mix(violet, mix(electric, white, signal * 0.34), signal), signal);
}
...
void main() {
  ...
  // Orbital architecture: independently rotating ellipses.
  ...
  // Crosshair and measurement system.
  float crosshair = 0.0;
  ...
  // OPEN state compresses the image into a bright portal.
