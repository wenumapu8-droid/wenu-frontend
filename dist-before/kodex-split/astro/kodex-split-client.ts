...
interface RuntimeState {
  ...
  branchBias: number;
  targetBranchBias: number;
  branchPulseStartedAt: number;
...
export function mountKodexSplitCorridor(
  root: HTMLElement,
): () => void {
  const canvas =
  ...
  if (!canvas) {
    throw new Error(
      "KDX_SPLIT_CORRIDOR: canvas not found.",
    );
  }
  ...
  const init =
    async (): Promise<void> => {
      const [
        ...
        fetch(
          `${shaderBase}/split-corridor.frag.glsl`,
        ).then((response) => {
          if (!response.ok) {
            throw new Error(
