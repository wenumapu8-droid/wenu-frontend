...
interface RuntimeState {
  ...
  creaseOrigin: [number, number];
  creaseStartedAt: number;
...
export function mountKodexWrinkledReality(
  root: HTMLElement,
): () => void {
  const canvas =
  ...
  if (!canvas) {
    throw new Error(
      "KDX_WRINKLED_REALITY: canvas not found.",
    );
  }
  ...
  const init =
    async (): Promise<void> => {
      const [
        ...
        fetch(
          `${shaderBase}/wrinkled-reality.frag.glsl`,
        ).then((response) => {
          if (!response.ok) {
            throw new Error(
