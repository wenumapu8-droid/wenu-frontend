...
void main(){
  ...
  o=vec4(base,1.);
}`,U=matchMedia("(prefers-reduced-motion: reduce)").matches;function m(){for(const a of document.querySelectorAll("[data-kdx-campo]")){if(a.dataset.kdxCampoMontado)continue;a.dataset.kdxCampoMontado="1";const e=a.getContext("webgl2",{antialias:!1,alpha:!0});if(!e)continue;const f=(t,d)=>{const o=e.createShader(t);return e.shaderSource(o,d),e.compileShader(o),e.getShaderParameter(o,e.COMPILE_STATUS)||console.error(e.getShaderInfoLog(o)),o},s=e.createProgram();e.attachShader(s,f(e.VERTEX_SHADER,I)),e.attachShader(s,f(e.FRAGMENT_SHADER,q)),e.linkProgram(s),e.useProgram(s);const w=e.createBuffer();e.bindBuffer(e.ARRAY_BUFFER,w),e.bufferData(e.ARRAY_BUFFER,new Float32Array([-1,-1,3,-1,-1,3]),e.STATIC_DRAW);const p=e.getAttribLocation(s,"p");e.enableVertexAttribArray(p),e.vertexAttribPointer(p,2,e.FLOAT,!1,0,0);const r=t=>e.getUniformLocation(s,t),E=r("u_res"),y=r("u_time"),R=r("u_seed"),S=r("u_pointer"),M=r("u_bass"),P=r("u_state"),C=r("u_px"),k=r("u_exp");e.uniform1f(R,Math.random());const L=performance.now(),i=Math.min(window.devicePixelRatio||1,2);let h=0,v=0;const b=()=>{h=a.clientWidth,v=a.clientHeight,a.width=Math.max(1,Math.round(h*i)),a.height=Math.max(1,Math.round(v*i)),e.viewport(0,0,a.width,a.height)};b(),addEventListener("resize",b,{passive:!0});let c=[0,0];addEventListener("pointermove",t=>{c=[(t.clientX/innerWidth-.5)*2,(.5-t.clientY/innerHeight)*2]},{passive:!0});const u=Number(a.dataset.kdxCampoEstado||"0.15"),O={idle:.1,aware:.24,locked:.46,active:.72,transitionOut:.9},T=()=>O[document.documentElement.dataset.kdxState||""],l=window.__kdxCampo||={bass:0,estado:u,exposicion:.52};let F=performance.now(),x=0,_=!0,n=u;const g=new IntersectionObserver(t=>{_=t[0].isIntersecting},{threshold:0});g.observe(a);const A=()=>{if(x=requestAnimationFrame(A),!_||document.hidden||!a.isConnected)return;const t=(performance.now()-F)/1e3;Math.max(2,Math.min(5,Math.round(Math.min(a.width,a.height)/260))),e.uniform2f(E,a.width,a.height),e.uniform1f(y,U?0:t),e.uniform2f(S,c[0],c[1]),e.uniform1f(M,l.bass||0);const d=T()??l.estado??u;n+=(d-n)*.035,e.uniform1f(P,n),e.uniform1f(C,(5+(1-n)*4)*i);const o=(performance.now()-L)/8e3,D=o>=1?1:o*o*(3-2*o);e.uniform1f(k,(l.exposicion??1)*(.06+.94*D)),e.drawArrays(e.TRIANGLES,0,3)};A(),document.addEventListener("astro:before-swap",()=>{a.isConnected||(cancelAnimationFrame(x),g.disconnect())})}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",m,{once:!0}):m();document.addEventListener("astro:page-load",m);
