...
in vec2 p; out vec2 v_uv; void main(){ v_uv=p*0.5+0.5; gl_Position=vec4(p,0.,1.); }`,W=`#version 300 es
...
void main(){
  ...
  o=vec4(base,1.);
}`,E=matchMedia("(prefers-reduced-motion: reduce)").matches;function f(){for(const a of document.querySelectorAll("[data-kdx-campo]")){if(a.dataset.kdxCampoMontado)continue;a.dataset.kdxCampoMontado="1";const e=a.getContext("webgl2",{antialias:!1,alpha:!0});if(!e)continue;const p=(t,n)=>{const s=e.createShader(t);return e.shaderSource(s,n),e.compileShader(s),e.getShaderParameter(s,e.COMPILE_STATUS)||console.error(e.getShaderInfoLog(s)),s},r=e.createProgram();e.attachShader(r,p(e.VERTEX_SHADER,B)),e.attachShader(r,p(e.FRAGMENT_SHADER,W)),e.linkProgram(r),e.useProgram(r);const y=e.createBuffer();e.bindBuffer(e.ARRAY_BUFFER,y),e.bufferData(e.ARRAY_BUFFER,new Float32Array([-1,-1,3,-1,-1,3]),e.STATIC_DRAW);const h=e.getAttribLocation(r,"p");e.enableVertexAttribArray(h),e.vertexAttribPointer(h,2,e.FLOAT,!1,0,0);const o=t=>e.getUniformLocation(r,t),R=o("u_res"),S=o("u_time"),M=o("u_seed"),P=o("u_pointer"),C=o("u_bass"),k=o("u_state"),L=o("u_px"),O=o("u_exp");e.uniform1f(M,Math.random());const T=performance.now(),c=Math.min(window.devicePixelRatio||1,2);let v=0,b=0;const x=()=>{v=a.clientWidth,b=a.clientHeight,a.width=Math.max(1,Math.round(v*c)),a.height=Math.max(1,Math.round(b*c)),e.viewport(0,0,a.width,a.height)};x(),addEventListener("resize",x,{passive:!0});let u=[0,0];addEventListener("pointermove",t=>{u=[(t.clientX/innerWidth-.5)*2,(.5-t.clientY/innerHeight)*2]},{passive:!0});const l=Number(a.dataset.kdxCampoEstado||"0.15"),F={idle:.1,aware:.24,locked:.46,active:.72,transitionOut:.9},D=()=>F[document.documentElement.dataset.kdxState||""],d=window.__kdxCampo||={bass:0,estado:l,exposicion:.52};let I=performance.now(),_=0,g=!0,m=l;const A=new IntersectionObserver(t=>{g=t[0].isIntersecting},{threshold:0});A.observe(a);const w=()=>{if(_=requestAnimationFrame(w),!g||document.hidden||!a.isConnected)return;const t=(performance.now()-I)/1e3,n=Math.max(2,Math.min(5,Math.round(Math.min(a.width,a.height)/260))),s=E?n*3:n;e.uniform2f(R,a.width,a.height),e.uniform1f(S,E?0:t),e.uniform2f(P,u[0],u[1]),e.uniform1f(C,d.bass||0);const q=D()??d.estado??l;m+=(q-m)*.035,e.uniform1f(k,m),e.uniform1f(L,s*c);const i=(performance.now()-T)/8e3,U=i>=1?1:i*i*(3-2*i);e.uniform1f(O,(d.exposicion??.52)*(.06+.94*U)),e.drawArrays(e.TRIANGLES,0,3)};w(),document.addEventListener("astro:before-swap",()=>{a.isConnected||(cancelAnimationFrame(_),A.disconnect())})}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",f,{once:!0}):f();document.addEventListener("astro:page-load",f);
